...
export function SignupForm() {
  const { signUp, signInWithGoogle, loading: authLoading, error: authError } = useAuth();
  ...
  const onSubmit: SubmitHandler<SignupFormInputs> = async (data) => {
    ...
    if (user) {
      toast({ title: "Signup Successful", description: "Welcome to WanderWise!" });
      router.push("/plan"); 
    } else {
      toast({
        variant: "destructive",
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (user) {
      ...
      toast({
        ...
        description: authError?.message || "An unexpected error occurred with Google Sign-Up. Please try again.",
      });
    }
  };
